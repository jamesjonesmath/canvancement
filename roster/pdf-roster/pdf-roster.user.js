...
// @require     https://cdn.rawgit.com/bpampuch/pdfmake/master/build/pdfmake.min.js
...
(function() {
  ...
  checkElement('table.roster', 'content', main);
  function checkElement(sel, parentId, callback) {
    if (document.querySelector(sel)) {
      callback();
    } else {
      var dst = document.getElementById(parentId);
      var observer = new MutationObserver(function() {
        if (dst.querySelector(sel)) {
          observer.disconnect();
          callback();
        }
      });
  ...
  function fetchRoster() {
    ...
    try {
      ...
      $('table.roster tbody tr').each(function() {
  ...
  function rosterHeader() {
    var courseName = ENV.CONTEXTS.courses[ENV.course.id].name;
    var termName = ENV.CONTEXTS.courses[ENV.course.id].term;
    var d = new Date();
    var heading = {
      'columns' : [ {
        'text' : [ courseName + ' ', {
  ...
  function confirmAllLoaded() {
    ...
    var rosterBody = document.querySelector('table.roster tbody');
    if (rosterBody.rows.length == 50) {
      var mesg = 'You have exactly 50 names in your roster.\n\n';
      mesg += 'If you are expecting more names, then you probably need to scroll down on the page to finish loading the entire roster.\n\n';
      mesg += 'Press OK to continue with the report or Cancel to return to the Roster Page.';
  ...
  function addRosterPdfButton() {
    if ($('#jj_roster_pdf').length === 0) {
      $('#people-options>ul').append(
        '<li class="ui-menu-item" role="presentation"><a id="jj_roster_pdf" class="ui-corner-all" role="menuitem"><i class="icon-download"></i> Download Roster as PDF</a></li>');
      $('#jj_roster_pdf').one('click', rosterPdf);
    }
    return;
  }
  function rosterPdf() {
